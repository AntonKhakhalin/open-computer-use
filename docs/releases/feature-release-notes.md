...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-22 | macOS window2 原生实现（窗口管理 + 窗口定向动作） | AI agent 可以在 macOS 上按窗口精确操作多窗口应用：`list_windows` 枚举真实窗口（CGWindowID，title 依赖 Screen Recording 授权）、`get_window`/`get_window_state` 读取单窗口无障碍树与截图（screenshot id 生命周期管理，stale id 执行前拒绝）、`activate_window` 把指定窗口带到前台并回验、七个动作工具全部支持 `window` 定向与 `screenshotId` 坐标观察门禁（观察带截图 / bounds 未变 / 点在像素内），行为与 Windows 对齐，不再依赖 app 级 key-window 猜测。 | 新增 `WindowManagement.swift`（`CGWindowListCopyWindowInfo` 窗口目录；stale 窗口 → 进程退出 → deny 名单解析序；unminimize + `NSRunningApplication.activate` + AXRaise/AXMain/AXFocused 激活回验，无 AppleScript/shell、无额外 feature gate）；`ComputerUseService` window2 API（`get_window_state` pretty-printed JSON、`shot-{windowID}-{n}` 截图 id 缓存与成功动作失效、坐标观察门禁）、`AccessibilitySnapshot`/`InputSimulation` 窗口定向支持、dispatcher 路由 14-tool 面；新增 46 个单测（多窗口/同名窗口/stale id/进程退出/deny 名单/激活切换/截图 id 生命周期/终止中操作），fixture 多窗口与 4 条 window2 命令，smoke suite 新增 W1–W10（`OPEN_COMPUTER_USE_SMOKE_WINDOW2=1` 门控）；README（含 zh-CN）、ARCHITECTURE、SKILL、usage 参考文档同步更新。 |
