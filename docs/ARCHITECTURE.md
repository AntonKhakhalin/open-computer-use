...
- 动作型 tools 对普通 app 采用“非侵入优先，物理指针路径显式 opt-in”策略：
  ...
  - `CGEvent.postToPid` 定向发送键盘事件，避免为了 `type_text` / `press_key` 抢前台；`type_text` 会把文本按 Unicode extended grapheme cluster 聚合成小批量 `keyboardSetUnicodeString` 事件，避免中文标点、emoji / 代理对和组合字符被逐个 UTF-16 code unit 拆开后在 Electron 富文本输入框里乱序或变形。如果当前 focused element 的 `AXValue` 可设置，`type_text` 会优先按可编辑内容追加并写回 `AXValue`，这覆盖 Feishu / Electron 富文本输入框不可靠接收后台键盘事件的场景，并会过滤已知占位提示，避免把 placeholder 拼进草稿；settable 查询本身失败（如 -25205）时按“未确认可设置”处理并继续键盘回退链，而不是把原始 AX 查询错误直接抛给调用方；如果当前 focused element 不是可编辑文本目标，`type_text` 会报错要求先 click 文本输入区或使用 `set_value`，不再把无效果的后台键盘投递当成成功；`press_key` 的 xdotool parser 覆盖官方 binary key table 里常见的 `BackSpace`、`Page_Up`、`Prior` / `Next`、`F1...F12` 和 `KP_*` alias
...
- 2026-08 起协议面对齐官方 Codex `window2` API（以官方 `computer-use` 插件 `docs/api.md` 为准），在 9 个 legacy tool 之外新增 5 个官方同名 tool：`list_windows`、`get_window`、`launch_app`、`get_window_state`、`activate_window`（共 14 个）。`Window = {app, id, title}` 的 `id` 在 Windows 上是不透明 HWND 整数（macOS 为 CGWindowID，`launch_app` 已落地；Linux 为 X window id），动作调用时通过可选 `window` 参数（或 `window_id`）定位，未传时回退 legacy `app` key-window 语义。
...
- macOS / Linux 侧接口定义（`ToolDefinitions.swift` / Linux `toolDefinitions()`）已同步为同样的 14-tool 面。macOS `launch_app` 已实现原生路径（`AppDiscovery.launch`，NSWorkspace/LaunchServices）：app 名或 Bundle ID 查询、复用运行中实例、`NSWorkspace.OpenConfiguration.activates = false` 后台启动不抢焦点、返回 `pid`/`bundleIdentifier`/`name`/`windows[]`（`CGWindowID` + 可选 title，title 依赖 Screen Recording 授权）、沿用既有 app deny 名单（密码管理器），无额外 feature gate。macOS 其余 4 个 window2 tool（`list_windows`、`get_window`、`get_window_state`、`activate_window`）与七个动作工具的 `window`/`screenshotId`/坐标滚动参数已实现原生路径（`WindowManagement.swift` + `ComputerUseService`）：窗口目录基于 `CGWindowListCopyWindowInfo`（on-screen、layer 0、非零尺寸，`{app, id, title}`，`id` 即 CGWindowID，title 依赖 Screen Recording 授权、无授权时省略 title 但 id 必返）；窗口解析顺序为 stale-window（`staleWindowHandle(id): the window is no longer open; re-observe with list_windows.`）→ 宿主进程已退出（`NSRunningApplication(processIdentifier:)` 判空，`staleWindowHandle(id): the owning process is no longer running; re-observe with list_windows.`）→ 复用既有 app deny 名单（密码管理器）→ AX-identity 存活检查（`WindowIdentity`，方案见 `docs/design-docs/macos-window-identity.md`：关闭窗口与 AppKit helper 窗口会在全量 CGWindow 列表长期残留；当宿主 app 的 AX window 列表被完整读出、且 `_AXUIElementGetWindow`（私有符号，runtime dlopen/dlsym 解析、缺失时 capability 降级、未公开 ABI 统一收敛在 `AXWindowIdentitySPI.swift`）证明没有窗口映射到该 id 时，按与 stale-window 相同的错误串报 stale；检查 indeterminate（符号缺失 / 无 AX window 列表 / 任一映射失败）时从不拒绝，不用 alpha/layer/bounds 等列表级启发式）。`get_window_state` / `activate_window` 的 AX window 匹配 identity-first：映射可用时按 CGWindowID 精确命中（same-bounds 窗口不再歧义，截图与 AX 树的错配从结构上排除）；映射不可用/不完整时回退 frame 几何 + title/focused tie-break，多个同 bounds 候选无法唯一确定时抛显式错误 `ambiguousWindow(id): ...` 而不是取第一个候选。`get_window_state` 默认 `include_screenshot=true` / `include_text=false`（两者皆关显式拒绝），结果为 pretty-printed JSON（`window` + `accessibility.tree`（`include_text` 开关的 `focused_element`/`selected_text`）+ `screenshots[]` 携带 `id/url/width/height/originX/originY/zIndex`，`width`/`height` 为窗口 bounds 点数），截图复用 `WindowCapture`/ScreenCaptureKit 既有抓取链；screenshot id 形如 `shot-{windowID}-{n}`，按窗口缓存、任何成功动作使其失效，stale id 在执行前拒绝（`stale screenshot id; re-observe with get_window_state before retrying.`），未观察窗口的 window 定向动作报 `No window state is available for window id ...`。window 定向坐标动作实施与 Windows 相同的观察门禁（`call get_window_state before issuing coordinate input`、`call get_window_state with include_screenshot before issuing coordinate input`、`window bounds changed; call get_window_state before continuing`、`(x, y) is outside screenshot bounds`），element 定向与 legacy `app` 键控流程不受影响。`activate_window` 走 unminimize → `NSRunningApplication.activate` → AXRaise + AXMain/AXFocused（≤3 次、0.25s 间隔）并对 frontmost app / 应用 focused window 回验（失败报 `activateWindow(id): the window could not be brought to the foreground; re-observe with list_windows.`），无 AppleScript/shell、无额外 feature gate。Linux 全部 5 个 window2 tool 仍返回明确的 "not supported yet" 错误。
